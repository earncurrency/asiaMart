...
const normalClass = 'flex items-center justify-center px-2 lg:px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 transition';
const activeClass = 'flex items-center justify-center px-2 lg:px-3 h-8 leading-tight text-gray-700 bg-gray-300 border border-gray-300 hover:bg-gray-100 hover:text-gray-700 transition';
</script>

<template>
    <!-- Pagination -->
    <nav class="flex items-center justify-center flex-wrap md:flex-row lg:justify-between pt-4 p-2"
        aria-label="Table navigation">
        <span
            class="text-sm font-normal text-gray-500 mb-4 md:mb-0 w-full md:inline md:w-auto text-center md:text-left">
            เเสดง <span class="font-semibold text-gray-900">{{ firstRowNumber }} - {{ lastRowNumber
                }}</span> จาก <span class="font-semibold text-gray-900">{{ totalList }}</span>
        </span>
        ...
        <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
            ...
            <li v-for="(index, idx) in pagination" :key="idx" @click="clickPage(index)">
                <a href="#" :class="index == currentPage ? activeClass : normalClass">{{ index }}</a>
            </li>
...
export default {
    ...
    computed: {
        ...
        pagination() {
            ...
            if (this.totalPagination <= 10) {
                ...
                for (let i = Math.max(2, this.currentPage - 2); i <= Math.min(this.totalPagination - 1, this.currentPage + 2); i++) {
                    pages.push(i);
                }
    ...
    methods: {
        ...
        clickPage(pageNo) {
            if (pageNo === '...' || pageNo < 1 || pageNo > this.totalPagination) return;
